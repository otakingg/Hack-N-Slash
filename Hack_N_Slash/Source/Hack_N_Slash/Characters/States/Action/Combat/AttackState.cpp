...
bool UAttackState::OnAttackIntent(const FVector2D& InputVector)
{
    ...
    // If this is the player, check and see if we can cancel
    ...
    else // AI will always be set to none state before they attack, so they don't need to check for cancel
    {
        iCombatCmd->AttackIntent(InputVector);
        return true;
    }
